...
func draw_line_relative_thick(point_a: Vector3, point_b: Vector3, thickness: float = 2.0, color: Color = Color.BLACK) -> void:
	...
	if draw_debug.mesh is ImmediateMesh:
		...
		var vertices_strip_order: Array[int] = [4, 5, 0, 1, 2, 5, 6, 4, 7, 0, 3, 2, 7, 6]
		var local_b: Vector3 = point_b - point_a
		
		for v in range(14):
			var vertex: Vector3 = normal if \
				vertices_strip_order[v] < 4 else \
				normal + local_b
			var final_vert: Vector3 = vertex.rotated(dir, PI * (0.5 * (vertices_strip_order[v] % 4) + 0.25))
